...
function sendPromptResponse(result) {
    var responseObj = { 'r': USERROUTE
                      , 'o': { 't': PROMPTRESULT
                             , 'result': result
                             }
                      };
...
function handleCanvasOut(canvasOut) {
    switch(canvasOut.t) {
        ...
        case MEASURETEXT:
            ...
            break;
        case PROMPT:
            performPrompt(canvasOut.a[0],canvasOut.a[1]);
            break;
        default:
...
function performPrompt(prompt, initval) {
    var result = window.prompt(prompt,initval);
    sendPromptResponse(result);
    logActivity("Prompt");
}
